...
- DOIs and final-publication years live in the sidecar `data/dois.csv` (`pubid,title,doi,year`), written by `scripts/enrich_dois.py` and joined at build time in `src/lib/publications.ts`. The `year` column is the Crossref publisher year and is preferred over the Scholar year (Scholar keeps the preprint year when it merges two-stage EGU journal papers). New publications arrive DOI-less; re-run `python3 scripts/enrich_dois.py` occasionally (safe to re-run; add `--loose` for a fuzzier second pass, and `--refresh-years` to backfill years for rows missing them) and commit the result.
