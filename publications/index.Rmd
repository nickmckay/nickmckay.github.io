...
if(nrow(recent_pubs) > 0) {
  for(i in 1:min(10, nrow(recent_pubs))) {
    ...
    if(grepl("4.2 ka event is not remarkable", pub$title)) {
      cat("📖 **[Read the story behind this research](/blog/4.2ka-event-not-remarkable/)**\n\n")
    }
    if(grepl("Globally coherent water cycle response", pub$title)) {
      cat("📖 **[Read the story behind this research](/blog/global-water-cycle-response/)**\n\n")
    }
    if(grepl("global Data Assimilation of Moisture Patterns", pub$title)) {
      cat("📖 **[Read the story behind this research](/blog/damp21ka-data-assimilation/)**\n\n")
    }
    if(grepl("Arctic glacier snowline altitudes", pub$title)) {
      cat("📖 **[Read the story behind this research](/blog/arctic-snowline-rise/)**\n\n")
    }
...
pubs_formatted %>%
  ...
  kable(format = "html", escape = FALSE, 
        caption = "Complete publication list (automatically updated from Google Scholar)") %>%
  kable_styling(
