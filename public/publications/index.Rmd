...
if(nrow(recent_pubs) > 0) {
  for(i in 1:min(10, nrow(recent_pubs))) {
    ...
    # Get full publication details to access complete author list
    full_authors <- tryCatch({
      pub_details <- get_publication_cite(scholar_id, pub$pubid)
      pub_details$author
    }, error = function(e) {
      pub$author
    })
    
    # Use full author list if available, otherwise fall back to truncated version
    authors_to_display <- if(!is.null(full_authors) && full_authors != "" && !is.na(full_authors)) full_authors else pub$author
